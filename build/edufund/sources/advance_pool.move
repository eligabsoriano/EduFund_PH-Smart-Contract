...
module edufund::advance_pool {
    ...
    entry fun deposit_and_keep<T>(pool: &mut AdvancePool<T>, coin: Coin<T>, ctx: &mut TxContext) {
        let amt = coin.value();
        pool.balance.join(coin.into_balance());
        event::emit(LiquidityAdded<T> { amount: amt, provider: ctx.sender() });
        let receipt = LPReceipt<T> { id: object::new(ctx), amount: amt };
        transfer::transfer(receipt, ctx.sender());
    }

    entry fun withdraw_with_receipt<T>(pool: &mut AdvancePool<T>, receipt: LPReceipt<T>, ctx: &mut TxContext) {
    ...
    entry fun take_advance<T>(pool: &mut AdvancePool<T>, amount: u64, ctx: &mut TxContext) {
    ...
    public(package) fun return_funds<T>(pool: &mut AdvancePool<T>, coin: Coin<T>) {
        pool.balance.join(coin.into_balance());
    }
